...
| `app/bao-cao/[slug]/page.tsx` | Server (async) | ✅ `await params`, slug = `driveDocId`. **BẢO VỆ PAYWALL**: server đọc cookie qua `getCurrentUser()` + `checkUserAccess()` — nếu `TRIAL_ACTIVE`/`SUBSCRIPTION_ACTIVE` → render `<ReportAudioPlayer/>` (Nghe đọc báo cáo — Web Speech API) + `<DriveDocViewer/>` iframe Google Docs; ngược lại (`UNAUTHENTICATED`/`EXPIRED`) → render `<Paywall status>` thay nội dung tài liệu (không thể bypass từ client). |
...
| `app/api/reports/[id]/content/route.ts` | Route handler | ✅ **API nội dung báo cáo (TTS)**: export Google Doc `text/plain` (`drive/v3/files/{id}/export?mimeType=text/plain&key=GOOGLE_DRIVE_API_KEY`) → `{content}`; `force-dynamic`, cache no-store; 400 thiếu id / 500 thiếu key. |
...
| `report-audio-player.tsx` | Client | ✅ **Nghe đọc báo cáo** (Web Speech API): `ReportAudioPlayer{docId}` fetch `/api/reports/[id]/content`; `speechSynthesis` `lang=vi-VN`, **luôn gán voice tiếng Việt** (`pickVietnameseVoice` ưu tiên "Google tiếng Việt") chống đọc tiếng Anh; `chunkText` tách câu max 200 ký tự + **bảo vệ dấu chấm/phẩy trong số** (28.500 không bị cắt); đọc nối tiếp qua `onend` + keepalive 10s (chống Chromium/Safari tự dừng ~15s); nút Phát/Tạm dừng/Dừng (Button ui + lucide), tốc độ 1.0x/1.5x/2.0x đổi tức thì, thanh tiến trình, style emerald. |

### 3.3 Libraries (`lib/`)

`data.ts` (14 mã + sectors + hàm định giá) · `report.ts` (BCTC JSON + MDX + RESEARCH_REPORTS) · `report-stocks.ts` (ghép dữ liệu bảng, có `bonusWelfareRate`) · `use-reports.ts` (hook + type `Report` có `bonusWelfareRate?`) · `format.ts` (fmtPrice/fmtNum/fmtInt/fmtPct/fmtBillion) · `utils.ts` (`cn()`) · **`auth-check.ts`** (logic thuần dùng chung client/server: `AccessStatus`, `UserProfile`, `TRIAL_DAYS=7`, `checkUserAccess()`, `canAccessReport()`, `SESSION_COOKIE_NAME`) · **`session.ts`** (SERVER-ONLY — import `next/headers`; `getCurrentUser()`, `setSessionCookie()` httpOnly, `clearSessionCookie()`, `parseUserProfile()`).
...
---
*Cập nhật lần cuối: 2026-08-23 · Người duy trì: Nguyễn Trung Nhật (trungnhat232@gmail.com)*
