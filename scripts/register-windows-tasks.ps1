...
Register-ScheduledTask -TaskName "StockData-Insider-1206" -Action $action1 -Trigger $trigger1 -Settings $settings -Description "Cập nhật Giao dịch nội bộ lúc 12:06 hàng ngày" -Force
...
Register-ScheduledTask -TaskName "StockData-Valuation-1616" -Action $action2 -Trigger $trigger2 -Settings $settings -Description "Cập nhật định giá P/E, P/B, Điểm 360 lúc 16:16 hàng ngày" -Force
...
Register-ScheduledTask -TaskName "StockData-Insider-1836" -Action $action3 -Trigger $trigger3 -Settings $settings -Description "Cập nhật Giao dịch nội bộ lúc 18:36 hàng ngày" -Force
Write-Host "  -> [OK] Đã đăng ký tác vụ: StockData-Insider-1836 (18:36 tối hàng ngày)" -ForegroundColor Green

# 4. Báo cáo phân tích Cổ phiếu & Ngành: 20:00 tối hàng ngày (1 ngày 1 lần)
$action4 = New-ScheduledTaskAction -Execute $nodePath -Argument "`"$scriptDir\sync-daily-reports.mjs`"" -WorkingDirectory $workingDir
$trigger4 = New-ScheduledTaskTrigger -Daily -At 20:00
Register-ScheduledTask -TaskName "StockData-AnalystReports-2000" -Action $action4 -Trigger $trigger4 -Settings $settings -Description "Cập nhật Báo cáo phân tích cổ phiếu & ngành lúc 20:00 hàng ngày" -Force
Write-Host "  -> [OK] Đã đăng ký tác vụ: StockData-AnalystReports-2000 (20:00 tối hàng ngày)" -ForegroundColor Green

# 5. Cổ đông lớn & Công ty con: 23:00 ngày 1 hàng tháng
$action5 = New-ScheduledTaskAction -Execute $nodePath -Argument "`"$scriptDir\crawl-direct-market.mjs`" --mode=shareholders" -WorkingDirectory $workingDir
$trigger5 = New-ScheduledTaskTrigger -Daily -At 23:00
Register-ScheduledTask -TaskName "StockData-Shareholders-Monthly" -Action $action5 -Trigger $trigger5 -Settings $settings -Description "Cập nhật Cổ đông & Công ty con ngày 1 hàng tháng" -Force
