 ...
 *   3. 16:25 hàng ngày: Cập nhật Hiệu suất Biến động giá (1W, 1M, 3M, 6M, 1Y, 3Y, YTD)
 *   4. 18:36 hàng ngày: Cập nhật Giao dịch nội bộ Lần 2 (Chốt tin chiều & cả ngày)
 *   5. 23:00 ngày 1 hàng tháng: Cập nhật Cơ cấu cổ đông lớn & Công ty con
 *
 * Cách chạy:
...
function checkAndRunSchedule() {
  ...
  // 3b. Biến động giá 1W, 1M, 3M, 6M, 1Y, 3Y, YTD: 16:25 hàng ngày (Sau khi chốt giá ATC)
  if (hours === 16 && minutes === 25 && lastRunPriceChangesDate !== dateStr) {
    lastRunPriceChangesDate = dateStr;
    runCommand("node scripts/sync-price-changes.mjs", "HIỆU SUẤT BIẾN ĐỘNG GIÁ (16:25 CHIỀU)");
  }
