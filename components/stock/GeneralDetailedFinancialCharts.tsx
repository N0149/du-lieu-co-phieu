...
// ── 10. Biểu đồ Biên Lợi Nhuận Gộp & Ròng ──
const ProfitMarginChartCard = React.memo(function ProfitMarginChartCard({
  data,
  isQuarter,
  globalQuarter,
  latest,
  ...
  return (
    <div
      onMouseLeave={handleMouseLeave}
      className="flex flex-col justify-between rounded-2xl border border-border/70 bg-card/90 p-4 shadow-xs transition-colors hover:border-emerald-500/40"
    >
      <div className="flex items-center justify-between border-b border-border/50 pb-2 mb-2">
        <div className="flex items-center gap-2">
          <span className="text-xs font-black uppercase text-foreground">Biên Lợi Nhuận Gộp & Ròng</span>
          {activeQuarter && isQuarter && (
            <span className="text-[10px] text-amber-400 font-bold bg-amber-500/10 px-1.5 py-0.5 rounded border border-amber-500/30">
              Q{activeQuarter}
            </span>
        ...
        <span className="font-mono text-[11px] font-bold text-amber-400">
          Gộp: {latest?.bienGop != null ? `${latest.bienGop.toFixed(1)}%` : '—'}
        </span>
      </div>
      <div className="h-[220px] w-full">
        <ResponsiveContainer width="100%" height={220}>
          ...
          >
            <CartesianGrid strokeDasharray="3 3" stroke="#334155" opacity={0.25} />
            <XAxis dataKey="displayDate" tick={tickRenderer || { fontSize: 9, fill: '#888' }} />
            ...
            <Line
              ...
              dot={(props: any) => {
                ...
                const isMatch = activeQuarter != null && q === activeQuarter
                if (!isMatch) return null
                return <circle cx={cx} cy={cy} r={5} fill="#10b981" stroke="#ffffff" strokeWidth={2} />
              }}
            />
            <Line
              type="monotone"
              ...
              dot={(props: any) => {
                ...
                const isMatch = activeQuarter != null && q === activeQuarter
                if (!isMatch) return null
                return <circle cx={cx} cy={cy} r={5} fill="#f59e0b" stroke="#ffffff" strokeWidth={2} />
              }}
            />
          </LineChart>
        </ResponsiveContainer>
...
// ── 11. Biểu đồ ROE & ROA ──
const RoeRoaChartCard = React.memo(function RoeRoaChartCard({
  data,
  isQuarter,
  globalQuarter,
  latest,
  ...
  return (
    <div
      onMouseLeave={handleMouseLeave}
      className="flex flex-col justify-between rounded-2xl border border-border/70 bg-card/90 p-4 shadow-xs transition-colors hover:border-sky-500/40"
    >
      <div className="flex items-center justify-between border-b border-border/50 pb-2 mb-2">
        <div className="flex items-center gap-2">
          <span className="text-xs font-black uppercase text-foreground">ROE & ROA Cuối Kỳ</span>
          {activeQuarter && isQuarter && (
            <span className="text-[10px] text-amber-400 font-bold bg-amber-500/10 px-1.5 py-0.5 rounded border border-amber-500/30">
              Q{activeQuarter}
            </span>
...
// ── 12. Biểu đồ TSCĐ & Khấu Hao ──
const FixedAssetChartCard = React.memo(function FixedAssetChartCard({
  data,
  isQuarter,
  globalQuarter,
  latest,
  ...
  return (
    <div
      onMouseLeave={handleMouseLeave}
      className="flex flex-col justify-between rounded-2xl border border-border/70 bg-card/90 p-4 shadow-xs transition-colors hover:border-blue-500/40"
    >
      <div className="flex items-center justify-between border-b border-border/50 pb-2 mb-2">
        <div className="flex items-center gap-2">
          <span className="text-xs font-black uppercase text-foreground">TSCĐ & Khấu Hao</span>
          {activeQuarter && isQuarter && (
            <span className="text-[10px] text-amber-400 font-bold bg-amber-500/10 px-1.5 py-0.5 rounded border border-amber-500/30">
              Q{activeQuarter}
            </span>
        ...
        <span className="font-mono text-[11px] text-muted-foreground">
          Đã KH: {latest?.pctKhauHao != null ? `${latest.pctKhauHao.toFixed(0)}%` : '—'}
        </span>
      </div>
      <div className="h-[220px] w-full">
        <ResponsiveContainer width="100%" height={220}>
          ...
          >
            <CartesianGrid strokeDasharray="3 3" stroke="#334155" opacity={0.25} />
            <XAxis dataKey="displayDate" tick={tickRenderer || { fontSize: 9, fill: '#888' }} />
            <YAxis yAxisId="left" tick={{ fontSize: 9, fill: '#888' }} />
            <YAxis yAxisId="right" orientation="right" tick={{ fontSize: 9, fill: '#888' }} unit="%" />
            <Tooltip content={<CustomChartTooltip />} isAnimationActive={false} animationDuration={0} cursor={{ fill: 'rgba(255, 255, 255, 0.05)' }} />
            <Legend wrapperStyle={{ fontSize: '9.5px' }} />
            <Bar yAxisId="left" dataKey="nguyenGia" name="Nguyên giá" radius={[2, 2, 0, 0]} isAnimationActive={false}>
              {data.map((entry, index) => {
                const q = getQuarter(entry)
                const isMatch = activeQuarter == null || q === activeQuarter
                return (
            ...
            <Bar yAxisId="left" dataKey="khauHao" name="Khấu hao" radius={[2, 2, 0, 0]} isAnimationActive={false}>
              {data.map((entry, index) => {
                const q = getQuarter(entry)
                const isMatch = activeQuarter == null || q === activeQuarter
                return (
            ...
            <Line yAxisId="right" type="monotone" dataKey="pctKhauHao" name="% Khấu hao" stroke="#f59e0b" strokeWidth={1.8} dot={false} isAnimationActive={false} />
          </ComposedChart>
        </ResponsiveContainer>
      </div>
    </div>
...
// ── 13. Biểu đồ Vòng Quay Tồn Kho ──
const InventoryTurnoverChartCard = React.memo(function InventoryTurnoverChartCard({
  data,
  isQuarter,
  globalQuarter,
  latest,
  ...
  return (
    <div
      onMouseLeave={handleMouseLeave}
      className="flex flex-col justify-between rounded-2xl border border-border/70 bg-card/90 p-4 shadow-xs transition-colors hover:border-purple-500/40"
    >
      <div className="flex items-center justify-between border-b border-border/50 pb-2 mb-2">
        <div className="flex items-center gap-2">
          <span className="text-xs font-black uppercase text-foreground">Vòng Quay Tồn Kho</span>
          {activeQuarter && isQuarter && (
            <span className="text-[10px] text-amber-400 font-bold bg-amber-500/10 px-1.5 py-0.5 rounded border border-amber-500/30">
              Q{activeQuarter}
            </span>
      ...
      <div className="h-[220px] w-full">
        <ResponsiveContainer width="100%" height={220}>
          ...
          >
            ...
            <XAxis dataKey="displayDate" tick={tickRenderer || { fontSize: 9, fill: '#888' }} />
            <YAxis yAxisId="left" tick={{ fontSize: 9, fill: '#888' }} />
            <YAxis yAxisId="right" orientation="right" tick={{ fontSize: 9, fill: '#888' }} unit="lần" />
            <Tooltip content={<CustomChartTooltip />} isAnimationActive={false} animationDuration={0} cursor={{ fill: 'rgba(255, 255, 255, 0.05)' }} />
            <Legend wrapperStyle={{ fontSize: '9.5px' }} />
            <Bar yAxisId="left" dataKey="tsTonKho" name="Tồn kho (Tỷ)" radius={[2, 2, 0, 0]} isAnimationActive={false}>
              {data.map((entry, index) => {
                const q = getQuarter(entry)
                const isMatch = activeQuarter == null || q === activeQuarter
                return (
                  <Cell
                    key={`tonkho-bar-${index}`}
                    fill="#8b5cf6"
                    fillOpacity={isMatch ? 1 : 0.15}
                    stroke={activeQuarter != null && isMatch ? '#ffffff' : 'none'}
                    strokeWidth={activeQuarter != null && isMatch ? 1.5 : 0}
            ...
            <Line yAxisId="right" type="monotone" dataKey="vongQuayTonKho" name="Vòng quay (lần)" stroke="#10b981" strokeWidth={1.8} dot={false} isAnimationActive={false} />
          </ComposedChart>
        </ResponsiveContainer>
      </div>
    </div>
...
export function GeneralDetailedFinancialCharts({
  ...
  return (
    <div className="w-full space-y-6">
      ...
      {/* ══════════════════════════════════════════════════════════ */}
      {/* KHỐI 4: CHỈ SỐ VẬN HÀNH & HIỆU QUẢ TÀI CHÍNH BỔ SUNG       */}
      {/* ══════════════════════════════════════════════════════════ */}
      <div className="space-y-4 pt-4 border-t border-border/60">
        <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-muted-foreground">
          <Layers className="size-4 text-sky-400" />
          <span>Chỉ số Hiệu Quả Hoạt Động & Vận Hành Bổ Sung</span>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-5">
          {/* Biểu đồ 10: Biên Lợi Nhuận Gộp & Ròng */}
          <ProfitMarginChartCard
            data={displayPoints}
            isQuarter={isQuarter}
            globalQuarter={globalLockedQuarter}
            latest={latest}
          ...
          {/* Biểu đồ 13: Vòng Quay Tồn Kho */}
          <InventoryTurnoverChartCard
            data={displayPoints}
            isQuarter={isQuarter}
            globalQuarter={globalLockedQuarter}
            latest={latest}
